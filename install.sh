...
echo "╔══════════════════════════════════════════════════════════╗"
echo "║          ProxyFarm Neo - IPv6 Proxy Server v6.0         ║"
echo "║          Прогрев прокси + фикс инструментов             ║"
echo "╚══════════════════════════════════════════════════════════╝"
...
def warmup_proxy(proxy):
    """Прогрев прокси для избежания 407 ошибки"""
    try:
        for _ in range(2):
            subprocess.run([
...
def check_website_via_proxy(proxy, url):
    try:
        result = subprocess.run([
            '/usr/bin/curl', '-x', f"http://{proxy['username']}:{proxy['password']}@{LOCAL_IPV4}:{proxy['port']}",
            '-s', '-o', '/dev/null', '-w', '%{http_code}', url, '--connect-timeout', '10', '--max-time', '15'
        ], capture_output=True, text=True, timeout=20)
...
def get_system_info():
    ...
    if PSUTIL:
        try:
            ...
            info['cpu'] = {'percent': round(cpu_percent, 1), 'count': cpu_count, 'freq_current': round(cpu_freq.current, 1) if cpu_freq else 0}
            ...
            info['system']['uptime'] = f"{h}ч {m}м {s}с"
            ...
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
                try:
                    ...
                    if pinfo['cpu_percent'] and pinfo['cpu_percent'] > 0.1:
                        info['processes'].append({'pid': pinfo['pid'], 'name': pinfo['name'][:25], 'cpu': round(pinfo['cpu_percent'], 1), 'memory': round(pinfo['memory_percent'] or 0, 1)})
                except: pass
...
def restart_3proxy_api():
    try:
        ...
        for p in load_proxies():
            warmup_proxy(p)
        return jsonify({'message': '3proxy перезапущен, прокси прогреты'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def speedtest():
    try:
        start = time.time()
        subprocess.run(['/usr/bin/curl', '-s', 'http://ipinfo.io/ip', '--connect-timeout', '5'], capture_output=True, timeout=10)
        return jsonify({'latency_ms': round((time.time() - start) * 1000, 2), 'status': 'ok'})
    except:
...
# index.html копируем текущий если есть, иначе базовый
if [ ! -f /opt/proxy-farm/templates/index.html ]; then
cat > /opt/proxy-farm/templates/index.html << 'HTMLEOF'
<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>ProxyFarm Neo</title>
...
for i in range(5):
    ...
    proxies.append({'id': f"p{port}", 'ipv6': ipv6, 'port': port, 'username': login, 'password': passwd, 'created_at': '2026-07-28T00:00:00', 'active': True, 'connection_format': f"62.148.226.89:{port}:{login}:{passwd}"})
