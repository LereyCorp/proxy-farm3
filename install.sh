...
clear
echo "╔══════════════════════════════════════════════════════════╗"
echo "║          ProxyFarm Neo - IPv6 Proxy Server v12.0        ║"
echo "║          Автоопределение + фикс HTML                    ║"
echo "╚══════════════════════════════════════════════════════════╝"
...
<!DOCTYPE html><html lang="ru"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>ProxyFarm Neo v12</title>
...
function changePassword(){var oldP=document.getElementById('oldPassword').value;var newP=document.getElementById('newPassword').value;if(!oldP||!newP){toast('Заполните оба поля','error');return}if(newP.length<6){toast('Пароль должен быть не менее 6 символов','error');return}api('/api/settings/change-password','POST',{old_password:oldP,new_password:newP},function(d){if(d.error){toast(d.error,'error')}else{toast('✅ '+d.message);document.getElementById('oldPassword').value='';document.getElementById('newPassword').value=''}})}
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║          УСТАНОВКА ЗАВЕРШЕНА! v12.0                     ║"
...
echo ""
echo "========================================="
echo "  ГОТОВО! v12.0 - Полный install.sh"
echo "  HTML шаблоны включены в установку"
echo "========================================="
